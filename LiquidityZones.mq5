//+------------------------------------------------------------------+
//|                                               LiquidityZones.mq5 |
//| Liquidity zones / zigzag detector with interactive panel.        |
//+------------------------------------------------------------------+
#property copyright   "2026"
#property version     "1.60"
#property description "Marks ATR-based liquidity zones with optional ZigZag fibo / RSI / candle filters"
...
input bool             InpUseDrawFibo      = false;
input double           InpMinDrawFibo      = 0.618;
input bool             InpUseDrawRSIDiv    = false;
input bool             InpUseDrawRSIExh    = false;
input bool             InpUseDrawCandles   = false;
...
#define PN_LDFILT  "LMCP_DFL"
#define PN_BDFIB   "LMCP_DFB"
#define PN_EDFIB   "LMCP_DFE"
#define PN_BDDIV   "LMCP_DDB"
#define PN_BDEXH   "LMCP_DEB"
#define PN_BDCND   "LMCP_DCB"
#define PN_DIV4    "LMCP_D4"
...
bool            g_drawUseFibo   = false;
double          g_drawMinFibo   = 0.618;
bool            g_drawUseDiv    = false;
bool            g_drawUseExh    = false;
bool            g_drawUseCandle = false;
...
int OnInit() {
    ...
    if(Period() > PERIOD_H1) {
        Alert("LiquidityZones: Attach to H1 or lower timeframe.");
        return INIT_FAILED;
    }
    g_atrHandle = iATR(_Symbol, PERIOD_D1, 14);
    if(g_atrHandle == INVALID_HANDLE) {
        Alert("LiquidityZones: Cannot create ATR(14,D1) handle.");
        return INIT_FAILED;
    }
    ...
    g_showZZLines   = true;
    g_drawUseFibo   = InpUseDrawFibo;
    g_drawMinFibo   = InpMinDrawFibo;
    g_drawUseDiv    = InpUseDrawRSIDiv;
    g_drawUseExh    = InpUseDrawRSIExh;
    g_drawUseCandle = InpUseDrawCandles;
    g_lastProcessed = 0;
    PurgeData();
    BuildPanel();
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
    ...
    if(id == CHARTEVENT_OBJECT_CLICK) {
        ...
        if(sparam == PN_BDRAW) {
            ...
            double f = StringToDouble(ObjectGetString(0, PN_EDFIB, OBJPROP_TEXT));
            if(v > 0) g_atrMult  = v;
            if(m > 0) g_rectMins = m;
            if(f > 0.0) g_drawMinFibo = f;
            if(g_useZigZag) {
        ...
        if(sparam == PN_BDFIB)  {
            double f = StringToDouble(ObjectGetString(0, PN_EDFIB, OBJPROP_TEXT));
            if(f > 0.0) g_drawMinFibo = f;
            g_drawUseFibo = !g_drawUseFibo;
            BuildPanel(); Redraw(); return;
        }
        if(sparam == PN_BDDIV)  { g_drawUseDiv    = !g_drawUseDiv;    BuildPanel(); Redraw(); return; }
        if(sparam == PN_BDEXH)  { g_drawUseExh    = !g_drawUseExh;    BuildPanel(); Redraw(); return; }
        if(sparam == PN_BDCND)  { g_drawUseCandle = !g_drawUseCandle; BuildPanel(); Redraw(); return; }
...
bool AnyDrawFilterEnabled() {
    return (g_drawUseFibo || g_drawUseDiv || g_drawUseExh || g_drawUseCandle);
}

bool ShouldDrawFilteredRect(bool atrQualified, bool hasFiboData, double ratio,
                            int divScore, int exhScore, int candleScore) {
    if(!atrQualified) return false;
    if(g_drawUseFibo   && (!hasFiboData || ratio < g_drawMinFibo)) return false;
    if(g_drawUseDiv    && divScore    <= 0) return false;
    if(g_drawUseExh    && exhScore    <= 0) return false;
    if(g_drawUseCandle && candleScore <= 0) return false;
    return true;
}

void DrawQualifiedZZRects(const double &zzBuf[], const MqlRates &bars[], int gotBars,
    ...
    for(int p = 0; p < pCount - 1; p++) {
        ...
        double leg = MathAbs(p0 - p1);
        bool atrQualified = (leg >= g_atrMult * atr);
        if(!atrQualified) continue;

        bool isHigh = (p0 > p1);
        bool hasFiboData = false;
        double ratio = 0.0;
        int divScore = 0;
        int exhScore = 0;
        int candleScore = 0;
        ...
        if(iN < rsiExhCnt) {
            ...
            if((isHigh && exhRsi > 70.0) || (!isHigh && exhRsi < 30.0))
                exhScore = 1;
        }

        string candleTip = "";
        candleScore = ScoreCandlestickPatterns(isHigh, iN, bars, gotBars, candleTip);

        if(ShouldDrawFilteredRect(atrQualified, hasFiboData, ratio, divScore, exhScore, candleScore))
            DrawZZRect(bars[iN].time, p0, p1);
    }
}
...
void ScanAndDrawZZ() {
    ...
    DrawQualifiedZZRects(zzBuf, bars, gotBars,
                         fastZZBuf,
...
bool EnsureZZHandle() {
    ...
    if(g_zzHandle == INVALID_HANDLE)
        Alert("LiquidityZones: Cannot create ZigZag handle. Compile Examples\\ZigZag first.");
...
bool EnsureFastZZHandle() {
    ...
    if(g_fastZZHandle == INVALID_HANDLE)
        Alert("LiquidityZones: Cannot create fast ZigZag handle. Compile Examples\\ZigZag first.");
...
bool EnsureRSIHandles() {
    ...
    if(g_rsiDivHandle == INVALID_HANDLE || g_rsiExhHandle == INVALID_HANDLE)
        Alert("LiquidityZones: Cannot create RSI handles.");
...
void BuildPanel() {
    ...
    string curFibo = (ObjectFind(0, PN_EDFIB)  >= 0) ? ObjectGetString(0, PN_EDFIB,  OBJPROP_TEXT) : DoubleToString(g_drawMinFibo, 3);
    PurgePanel();
    ...
    PnLabel(PN_TITLE, x+8, y+7, "Liquidity Zones", clrSilver, 8);
    ...
    // --- Draw filters ---
    PnLabel(PN_LDFILT, x+8, y+239, "Draw Filters:", clrSilver);
    color fibBg = g_drawUseFibo ? C'0,130,0' : C'65,65,65';
    color divBg = g_drawUseDiv ? C'0,130,0' : C'65,65,65';
    color exhBg = g_drawUseExh ? C'0,130,0' : C'65,65,65';
    color cndBg = g_drawUseCandle ? C'0,130,0' : C'65,65,65';
    PnButton(PN_BDFIB, x+8,  y+256, 78, 21, g_drawUseFibo ? "Fibo ON" : "Fibo OFF", fibBg);
    PnEdit (PN_EDFIB, x+90, y+258, 102, 18, curFibo);
    PnButton(PN_BDDIV, x+8,  y+282, PANEL_W-16, 21, g_drawUseDiv ? "RSI Div: ON" : "RSI Div: OFF", divBg);
    PnButton(PN_BDEXH, x+8,  y+308, PANEL_W-16, 21, g_drawUseExh ? "RSI Exh: ON" : "RSI Exh: OFF", exhBg);
    PnButton(PN_BDCND, x+8,  y+334, PANEL_W-16, 21, g_drawUseCandle ? "Candles: ON" : "Candles: OFF", cndBg);
    PnBG(PN_DIV3, x, y+361, PANEL_W, 1, C'70,70,70');

    // --- Fill toggle ---
    PnLabel(PN_LFILL, x+8, y+371, "Fill:", clrSilver);
    color  fillBg = g_fillOn ? C'0,130,0' : C'65,65,65';
    PnButton(PN_BFILL, x+90, y+366, 102, 21, g_fillOn ? "ON" : "OFF", fillBg);

    // --- Show ZigZag lines toggle ---
    PnLabel(PN_LSHOWZZ, x+8, y+397, "Show ZigZag:", clrSilver);
    color  zzLineBg = g_showZZLines ? C'0,130,0' : C'65,65,65';
    PnButton(PN_BSHOWZZ, x+90, y+392, 102, 21, g_showZZLines ? "ON" : "OFF", zzLineBg);
    PnBG(PN_DIV4, x, y+418, PANEL_W, 1, C'70,70,70');

    // --- Show filter ---
    PnLabel(PN_LSHOW, x+8, y+424, "Show:", clrSilver);
    ...
    PnButton(PN_BBOTH, x+8,          y+446, bw,   21, "Both",  cBoth);
    PnButton(PN_BBULL, x+8+bw+2,     y+446, bw,   21, "Red",   cBull);
    PnButton(PN_BBEAR, x+8+(bw+2)*2, y+446, bw+2, 21, "Green", cBear);
